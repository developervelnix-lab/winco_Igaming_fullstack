...
if (!function_exists('render_pagination')) {
    function render_pagination($current_page, $total_pages, $param_name, $anchor) {
        ...
        $html = '<nav class="mt-4"><ul class="pagination justify-content-center gap-2" style="border:none;">';
        ...
        $html .= '<li class="page-item ' . ($current_page <= 1 ? 'disabled' : '') . '">
                    <a class="page-link" href="' . $prev_link . '" style="background:var(--input-bg); border:1px solid var(--border-dim); color:var(--text-dim); border-radius:8px; padding:8px 16px; text-decoration:none;">Previous</a>
                  </li>';
        ...
        for ($i = 1; $i <= $total_pages; $i++) {
            if ($i == 1 || $i == $total_pages || ($i >= $current_page - 2 && $i <= $current_page + 2)) {
                ...
                $html .= '<li class="page-item ' . ($i == $current_page ? 'active' : '') . '">
                            <a class="page-link" href="' . $link . '" style="border-radius:8px; display:flex; align-items:center; justify-content:center; width:38px; height:38px; padding:0; text-decoration:none; ' . $active_style . '">' . $i . '</a>
                          </li>';
            } elseif ($i == $current_page - 3 || $i == $current_page + 3) {
                $html .= '<li class="page-item disabled"><span class="page-link" style="background:transparent; border:none; color:var(--text-dim);">...</span></li>';
            }
        }
        ...
        $html .= '<li class="page-item ' . ($current_page >= $total_pages ? 'disabled' : '') . '">
                    <a class="page-link" href="' . $next_link . '" style="background:var(--input-bg); border:1px solid var(--border-dim); color:var(--text-dim); border-radius:8px; padding:8px 16px; text-decoration:none;">Next</a>
                  </li>';
...
.dash-title { font-size: 26px; font-weight: 700; color: var(--text-main); font-family: var(--font-body); }
...
.r-table thead th { font-size: 11px; font-weight: 700; letter-spacing: 1.5px; text-transform: uppercase; color: var(--text-dim); padding: 0 16px 8px; border-bottom: 1px solid var(--border-dim); }
.r-table tbody td { padding: 14px 16px; font-size: 14px; background: var(--table-header-bg); border-top: 1px solid var(--border-dim); border-bottom: 1px solid var(--border-dim); color: var(--text-main); }
...
.badge-profit { background: rgba(16,185,129,0.15); color: var(--status-success); border: 1px solid rgba(16,185,129,0.3); border-radius: 20px; padding: 4px 12px; font-size: 11px; font-weight: 700; text-transform: uppercase; }
.badge-loss { background: rgba(239,68,68,0.15); color: var(--status-danger); border: 1px solid rgba(239,68,68,0.3); border-radius: 20px; padding: 4px 12px; font-size: 11px; font-weight: 700; text-transform: uppercase; }
...
.user-detail-item { display: flex; align-items: center; gap: 12px; margin-bottom: 10px; }
.user-detail-label { font-size: 12px; color: var(--text-dim); font-weight: 600; text-transform: uppercase; letter-spacing: 1px; width: 100px; }
.user-detail-value { font-size: 18px; font-weight: 700; color: var(--text-main); }
...
.dash-badge i { color: var(--status-info); font-size:14px; }
.dash-live-dot {
...
<div class="admin-layout-wrapper">
  ...
  <div class="admin-main-content hide-native-scrollbar">
    ...
    <div class="premium-card">
      <div class="d-flex align-items-center gap-2 mb-4">
        <div style="width: 4px; height: 24px; background: linear-gradient(180deg, var(--accent-teal), var(--accent-cyan)); border-radius: 4px;"></div>
        <h2 class="dash-title" style="font-size: 18px; margin: 0;">User Profile Summary</h2>
      </div>
    ...
    <div class="premium-card" id="recharge_section">
      <div class="d-flex align-items-center justify-content-between flex-wrap gap-3 mb-4">
        <div class="d-flex align-items-center gap-2">
          <div style="width: 4px; height: 24px; background: var(--accent-teal); border-radius: 4px;"></div>
          <h2 class="dash-title" style="font-size: 18px; margin: 0;">Recharge Activities</h2>
        </div>
      </div>
          ...
          <tbody>
            ...
            if (mysqli_num_rows($result) > 0){
              while ($row = mysqli_fetch_assoc($result)){
                ...
                ?>
                <tr>
                  <td style="font-weight: 600;"><?php echo $row['tbl_uniq_id']; ?></td>
    ...
    <div class="premium-card">
      <div class="d-flex align-items-center justify-content-between flex-wrap gap-3 mb-4">
        <div class="d-flex align-items-center gap-2">
          <div style="width: 4px; height: 24px; background: var(--accent-cyan); border-radius: 4px;"></div>
          <h2 class="dash-title" style="font-size: 18px; margin: 0;">Recharge Consolidated Report</h2>
        </div>
    ...
    <div class="premium-card" id="withdrawal_section">
      <div class="d-flex align-items-center justify-content-between flex-wrap gap-3 mb-4">
        <div class="d-flex align-items-center gap-2">
          <div style="width: 4px; height: 24px; background: #f43f5e; border-radius: 4px;"></div>
          <h2 class="dash-title" style="font-size: 18px; margin: 0;">Withdrawal Activities</h2>
        </div>
      </div>
    ...
    <div class="premium-card" id="withdrawal_summary_section">
      <div class="d-flex align-items-center justify-content-between flex-wrap gap-3 mb-4">
        <div class="d-flex align-items-center gap-2">
          <div style="width: 4px; height: 24px; background: #fb7185; border-radius: 4px;"></div>
          <h2 class="dash-title" style="font-size: 18px; margin: 0;">Withdrawal Consolidated Report</h2>
        </div>
        <div class="d-flex gap-2">
    ...
    <div class="premium-card" id="match_section">
      <div class="d-flex align-items-center justify-content-between flex-wrap gap-3 mb-4">
        <div class="d-flex align-items-center gap-2">
          <div style="width: 4px; height: 24px; background: #fca90e; border-radius: 4px;"></div>
          <h2 class="dash-title" style="font-size: 18px; margin: 0;">Match Activities</h2>
        </div>
        <div class="d-flex gap-2">
      ...
      <div class="table-responsive">
        <table id="match_activities_table" class="r-table">
          ...
          <tbody>
            ...
            <?php }else{ ?>
              <tr>
                <td colspan="5" class="text-center py-5" style="color: var(--text-dim);">No match records found.</td>
              </tr>
            <?php } ?>
          </tbody>
        </table>
    ...
    <div class="premium-card" id="sports_section">
      <div class="d-flex align-items-center justify-content-between flex-wrap gap-3 mb-4">
        <div class="d-flex align-items-center gap-2">
          <div style="width: 4px; height: 24px; background: #003366; border-radius: 4px;"></div>
          <h2 class="dash-title" style="font-size: 18px; margin: 0;">Sports Activities</h2>
        </div>
        <div class="d-flex gap-2">
      ...
      <?php
      $sports_where = "WHERE tbl_user_id='{$user_id}' AND (LOWER(tbl_project_name) IN ('saba sports', 'lucksport', 'lucksportgaming'))";
 ...
 <!--sports -->
    ...
    <div class="premium-card" id="match_summary_section">
      <div class="d-flex align-items-center justify-content-between flex-wrap gap-3 mb-4">
        <div class="d-flex align-items-center gap-2">
          <div style="width: 4px; height: 24px; background: #8B4513; border-radius: 4px;"></div>
          <h2 class="dash-title" style="font-size: 18px; margin: 0;">Match Activities Consolidated Report</h2>
        </div>
        <div class="d-flex gap-2">
    ...
    <div class="premium-card" id="summary_section">
      <div class="d-flex align-items-center justify-content-between flex-wrap gap-3 mb-4">
        <div class="d-flex align-items-center gap-2">
          <div style="width: 4px; height: 24px; background: var(--accent-teal); border-radius: 4px;"></div>
          <h2 class="dash-title" style="font-size: 18px; margin: 0;">User Summary Report (Financial)</h2>
        </div>
