...
/* Page specific variable overrides only if needed */
        ...
        .actions-bar {
            padding: 24px 20px; display: flex; gap: 12px; align-items: center; justify-content: space-between;
        }

        .btn-modern {
            height: 44px; padding: 0 20px; border-radius: 12px; font-weight: 600; font-size: 14px;
        ...
        .btn-outline-modern { background: var(--input-bg); border: 1px solid var(--border-dim); color: var(--text-dim); }
        ...
        .filter-options {
            ...
            border-radius: 16px; padding: 20px; margin: 0 20px 24px;
        }
        .cus-checkbox-group { display: flex; gap: 24px; align-items: center; margin-bottom: 16px; }
        ...
        .rank-badge {
            ...
            background: rgba(255,255,255,0.05); border: 1px solid var(--border-dim); color: var(--text-dim);
        }
        .rank-1 { background: linear-gradient(135deg, #fbbf24, #d97706) !important; border: none !important; color: #451a03 !important; box-shadow: 0 0 15px rgba(251, 191, 36, 0.4); }
        .rank-2 { background: linear-gradient(135deg, #94a3b8, #475569) !important; border: none !important; color: #fff !important; box-shadow: 0 0 15px rgba(148, 163, 184, 0.3); }
        .rank-3 { background: linear-gradient(135deg, #b45309, #78350f) !important; border: none !important; color: #fff !important; box-shadow: 0 0 15px rgba(180, 83, 9, 0.3); }
        ...
        .tag-dark { background: rgba(255,255,255,0.05); color: var(--text-dim); }

        .pagination-container { display: flex; justify-content: space-between; align-items: center; margin-top: 24px; padding: 0 10px; }
        .page-info { font-size: 12px; font-weight: 700; color: var(--text-dim); text-transform: uppercase; letter-spacing: 1px; }
    ...
    <div class="admin-main-content hide-native-scrollbar">
        ...
        <div class="dash-header">
            ...
            <div class="d-flex gap-2">
                <button class="btn-modern btn-outline-modern filter_btn"><i class='bx bx-filter'></i> Advanced Filter</button>
                <button class="btn-modern btn-primary-modern" onclick="exportPDF('top-balances', 'table')"><i class='bx bxs-file-pdf'></i> Export Data</button>
            </div>
        </div>
        ...
        <div class="filter-options hide_view">
            <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="POST">
                <p style="font-size: 11px; font-weight: 800; color: var(--accent-blue); text-transform: uppercase; margin-bottom: 16px; letter-spacing: 1px;">Account Status Filter</p>
                <div class="cus-checkbox-group">
                    <label class="cus-checkbox">
                        <input type="checkbox" name="order_type" value="true" <?php if($accountStatus=="true"){ echo 'checked'; } ?>>
                        <span>Show Active Players</span>
                    </label>
                    <label class="cus-checkbox">
                        <input type="checkbox" name="order_type" value="ban" <?php if($accountStatus=="ban"){ echo 'checked'; } ?>>
                        <span>Show Banned Players</span>
                    </label>
                    <label class="cus-checkbox">
                        <input type="checkbox" name="order_type" value="false" <?php if($accountStatus=="false"){ echo 'checked'; } ?>>
                        <span>Show In-Active Players</span>
                    </label>
                </div>
                <div class="d-flex gap-2">
                    <button type="submit" name="submit" class="btn-modern btn-primary-modern" style="height: 40px; font-size: 12px;">Apply Changes</button>
                    <button type="button" class="btn-modern btn-outline-modern filter_btn" style="height: 40px; font-size: 12px;">Cancel</button>
                </div>
            </form>
        </div>
        ...
        <div style="padding: 0 20px;">
            <div class="d-flex align-items-center justify-content-between mb-3">
                <p style="font-size: 14px; font-weight: 700; color: var(--text-main); margin: 0;">Ranking Overview</p>
                ...
                <tbody>
                    ...
                    if (mysqli_num_rows($user_records_result) > 0){
                        ...
                        while ($row = mysqli_fetch_assoc($user_records_result)){
                            ...
                            $rankClass = "";
                            if($indexVal == 1) $rankClass = "rank-1";
                            else if($indexVal == 2) $rankClass = "rank-2";
                            else if($indexVal == 3) $rankClass = "rank-3";
                    ?>
                        <tr onclick="window.location.href='manager.php?id=<?php echo $uniq_id; ?>'">
                            ...
                            <td>
                                <div style="font-weight: 800; color: var(--text-main);"><?php echo htmlspecialchars($username); ?></div>
                                <div style="font-size: 11px; color: var(--text-dim); font-family: monospace;">ID: <?php echo $uniq_id; ?></div>
                            </td>
                            <td style="text-align: right; font-weight: 800; color: var(--accent-emerald); font-size: 15px;">₹<?php echo number_format($balance, 2); ?></td>
                            <td style="text-align: center;">
                                <div style="font-size: 12px; font-weight: 700;"><?php echo $row['tbl_mobile_num']; ?></div>
                                <div style="font-size: 10px; color: var(--text-dim);"><i class='bx bx-wifi'></i> <?php echo $ip; ?></div>
                            </td>
                    ...
                    <?php $indexVal++; }} else { ?>
                        <tr>
                            <td colspan="6" style="text-align: center; padding: 60px; color: var(--text-dim);">
                                <i class='bx bx-list-minus' style="font-size: 48px; display: block; margin-bottom: 12px; opacity: 0.5;"></i>
            ...
            <div class="pagination-container">
                <div class="page-info">
                    Page <?php echo $page_num; ?> of <?php echo $total_page; ?> <span style="margin-left: 10px; opacity: 0.5;">•</span> Total <?php echo $total_records; ?> Players
                </div>
                <div class="d-flex gap-2">
                    <?php if ($page_num > 1): ?>
                        <a href="?page_num=<?php echo $page_num-1; ?>&order_type=<?php echo $accountStatus; ?>" class="btn-modern btn-outline-modern">
