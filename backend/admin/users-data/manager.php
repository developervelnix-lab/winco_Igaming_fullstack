...
/* Page specific variable overrides only if needed */
        ...
        .info-item { display: flex; justify-content: space-between; padding: 12px 0; border-bottom: 1px solid rgba(255,255,255,0.03); }
        .info-label { color: var(--text-dim); font-size: 13px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.5px; }
        .info-value { color: var(--text-main); font-weight: 700; font-size: 14px; }

        .actions-panel { display: flex; flex-direction: column; gap: 16px; }
        
        .btn-action {
            width: 100%; padding: 16px; border-radius: 14px; border: none; font-weight: 700;
            display: flex; align-items: center; justify-content: space-between; cursor: pointer;
            transition: all 0.2s; color: #fff; font-size: 15px; text-decoration: none;
        }
        .btn-action i { font-size: 22px; }
        .btn-action:hover { transform: translateY(-2px); box-shadow: 0 4px 15px rgba(0,0,0,0.3); }

        .btn-status-toggle { background: linear-gradient(135deg, var(--status-info), #2563eb); }
        .btn-status-ban    { background: linear-gradient(135deg, var(--status-danger), #e11d48); }
        .btn-status-active { background: linear-gradient(135deg, var(--status-success), #059669); }
        .btn-nav           { background: var(--input-bg); border: 1px solid var(--border-dim); color: var(--text-main); }
        .btn-nav:hover     { background: var(--table-row-hover); border-color: var(--status-info); }
    ...
    <div class="admin-main-content hide-native-scrollbar">
        ...
        <div class="manager-container">
            ...
            <div class="info-card">
                ...
                <div class="info-item"><span class="info-label">Mobile Number</span><span class="info-value"><?php echo $user_mobile_num; ?></span></div>
                <div class="info-item"><span class="info-label">Email ID</span><span class="info-value"><?php echo $user_email_id; ?></span></div>
                <div class="info-item"><span class="info-label">Main Balance</span><span class="info-value" style="color: var(--status-success);">₹<?php echo number_format($user_balance,2); ?></span></div>
                <div class="info-item"><span class="info-label">Rewards</span><span class="info-value" style="color: var(--status-warning);">₹<?php echo number_format($user_reward_balance,2); ?></span></div>
                <div class="info-item"><span class="info-label">Account Level</span><span class="info-value">Lv. <?php echo $account_level; ?></span></div>
                <div class="info-item"><span class="info-label">Referrer</span><span class="info-value"><?php echo $user_refered_by ?: 'Organic'; ?></span></div>
                <div class="info-item"><span class="info-label">Last Active</span><span class="info-value"><?php echo $user_last_active_date.' '.$user_last_active_time; ?></span></div>
                <div class="info-item"><span class="info-label">Join Date</span><span class="info-value"><?php echo $user_joined; ?></span></div>
            </div>
