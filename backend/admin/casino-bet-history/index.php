...
/* Page specific variable overrides only if needed */
        ...
        .type-debit    { color: #f43f5e; font-weight: 700; }
        .type-credit   { color: #10b981; font-weight: 700; }
        ...
        <div style="padding: 24px 14px;">
            ...
            <div class="search-area">
                <form method="POST" class="search-input-group">
                    <input type="text" name="searchinp" placeholder="Search Provider, Game Round ID, User ID..." class="search-input" value="<?php echo htmlspecialchars($searched); ?>" />
                            ...
                            if($searched!=""){
                                ...
                                $play_records_sql = "
                                    ...
                                    WHERE tbl_project_name IN (
                                        'Microgaming Lobby', 'Ezugi Lobby', 'Evolution Lobby',
                                        'XXXtreme Lightning Roulette', 'Crazy Time', 'Lightning Roulette',
                                        'MONOPOLY Live', 'Crazy Pachinko', 'Football Studio Dice',
                                        'Fan Tan', 'Speed Roulette', 'Craps', 'Super Andar Bahar',
                                        'Playtech Lobby', 'Power Blackjack', 'Infinite Blackjack',
                                        'Dead or Alive Saloon', 'Caribbean Stud Poker', 'Blackjack B',
                                        'Immersive Roulette', 'Baccarat B'
                                    )
                                    ORDER BY id DESC 
                            ...
                            if (mysqli_num_rows($play_records_result) > 0){
                                while ($row = mysqli_fetch_assoc($play_records_result)){
                                    ...
                                    <tr>
                                        ...
                                        <td style="font-family: monospace; font-size: 11px; color: var(--text-dim);"><?php echo htmlspecialchars($row['tbl_period_id']); ?></td>
                                        ...
                                        <td>
                                          <span class="status-badge <?php echo $is_profit ? 'status-profit' : 'status-loss'; ?>">
                                            ...
                                            <?php echo htmlspecialchars($row['tbl_match_result']); ?>
                                          </span>
                                        </td>
                                    </tr>
                                    <?php $indexVal++; 
                                ...
                                echo "<tr><td colspan='11' class='text-center py-5 text-muted'>No transaction records found</td></tr>";
                            } ?>
                        </tbody>
                        <?php if ($indexVal > 1) { ?>
                        <tfoot style="background: var(--table-header-bg);">
                            <tr>
                                <td colspan="8" style="text-align: right; font-weight: 700; color: var(--text-main);">Page Totals:</td>
                                <td style="font-weight: 800; color: var(--text-main);">₹<?php echo number_format($total_balance, 2); ?></td>
                                <td colspan="2">
