...
:root {
  --accent-blue:  var(--status-info);
        ...
        .chart-container {
            background: var(--panel-bg); border-radius: 20px; border: 1px solid var(--border-dim);
            padding: 24px; box-shadow: var(--card-shadow);
        }
        .chart-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 24px; }
        .chart-title { font-size: 14px; font-weight: 700; color: var(--text-main); text-transform: uppercase; letter-spacing: 1px; }

        .btn-modern {
            height: 36px; padding: 0 16px; border-radius: 10px; font-weight: 700; font-size: 12px;
            display: inline-flex; align-items: center; gap: 6px; transition: all 0.2s;
            cursor: pointer; border: none; background: var(--input-bg); color: var(--text-main);
        }
        .btn-modern:hover { background: var(--table-row-hover); }
    </style>
</head>
<body class="bg-light">
    ...
    <div class="admin-main-content hide-native-scrollbar">
        <div class="dash-header">
            <div class="dash-title">
                ...
                <h1>Withdraw Statistics</h1>
            </div>
            <div class="d-flex gap-2">
                <button class="btn-modern" onclick="window.location.reload()"><i class='bx bx-refresh'></i> Refresh Feed</button>
            </div>
        </div>
