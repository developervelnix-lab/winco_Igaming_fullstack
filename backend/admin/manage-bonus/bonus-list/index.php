                ...
                <table class="r-table">
                    ...
                    <tbody>
                        ...
                        if(!empty($_GET['code'])) {
                            ...
                            $where .= " AND coupon_code LIKE '%$code%'";
                        }
                        if(!empty($_GET['name'])) {
                        ...
                        if(!empty($_GET['redemption_type'])) {
                            $rt = mysqli_real_escape_string($conn, $_GET['redemption_type']);
                            $where .= " AND redemption_type = '$rt'";
                        }
                        if(!empty($_GET['affiliate_id'])) {
                            $aff = mysqli_real_escape_string($conn, $_GET['affiliate_id']);
                            $where .= " AND affiliate_id LIKE '%$aff%'";
                        }
                        ...
                        $res = mysqli_query($conn, "SELECT * FROM tbl_bonuses $where ORDER BY id DESC");
                        $row_num = 0;
                        while ($s = mysqli_fetch_assoc($res)) {
                            $row_num++;
                            $is_active = ($s['status'] == 'active' && (empty($s['end_at']) || strtotime($s['end_at']) >= time()));
                            ?>
                            <tr>
                                <td>
                                    <div class="d-flex align-items-center">
                                        <a href="../create-bonus/index.php?id=<?php echo $s['id']; ?>" class="btn-edit" title="Edit">
                                ...
                                <td><span style="font-family: monospace; font-weight: 600; color: var(--text-main);"><?php echo $s['coupon_code']; ?></span></td>
                                <td>
                                    <?php if ($is_active): ?>
                                ...
                                <td><?php echo $s['start_at'] ? date('d M Y, H:i', strtotime($s['start_at'])) : '-'; ?></td>
                                <td><?php echo $s['end_at'] ? date('d M Y, H:i', strtotime($s['end_at'])) : '-'; ?></td>
                            </tr>
                            <?php
                        }
                        if(mysqli_num_rows($res) == 0) {
                            echo "<tr><td colspan='7' class='text-center' style='padding: 40px; color: var(--text-dim);'>No bonuses found.</td></tr>";
                        }
                        ?>
                    </tbody>
                </table>
