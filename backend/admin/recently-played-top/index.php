...
$game_stats_sql = "SELECT mp.tbl_project_name as game, 
                   ...
                   WHERE DATE(mp.tbl_time_stamp) = '$today_date' 
...
$todayTopBetsSql = "SELECT * FROM tblmatchplayed 
                    WHERE DATE(tbl_time_stamp) = '$today_date' 
...
$yesterdayTopBetsSql = "SELECT * FROM tblmatchplayed 
                        WHERE DATE(tbl_time_stamp) = '$yesterday_date'
                        AND tbl_match_cost > 5000
                        ORDER BY tbl_match_cost DESC LIMIT 5";
...
$alt_yesterday_sql = "SELECT * FROM tblmatchplayed 
                     WHERE tbl_time_stamp >= NOW() - INTERVAL 2 DAY 
                     AND tbl_time_stamp < NOW() - INTERVAL 1 DAY
                     AND tbl_match_cost > 5000
                     ORDER BY tbl_match_cost DESC LIMIT 5";
...
/* Page specific variable overrides only if needed */
        ...
        .btn-modern {
            ...
            cursor: pointer; border: none; background: var(--input-bg); color: var(--text-main);
        }
        .btn-modern:hover { transform: translateY(-2px); background: var(--table-row-hover); }
        .btn-primary-modern { background: var(--accent-blue); color: #ffffff !important; }
        .btn-primary-modern:hover { box-shadow: 0 8px 20px rgba(59, 130, 246, 0.3); }
        .btn-outline-modern { background: var(--input-bg); border: 1px solid var(--border-dim); color: var(--text-main); }
        .btn-outline-modern:hover { border-color: var(--accent-blue); }

        .search-area {
            background: var(--panel-bg); border: 1px solid var(--border-dim); border-radius: 20px;
            padding: 24px; margin-bottom: 24px; box-shadow: var(--card-shadow);
        }
        .search-input-group { display: flex; gap: 12px; flex-wrap: wrap; }
        .search-input {
            flex-grow: 1; background: var(--input-bg); border: 1px solid var(--border-dim);
            border-radius: 12px; padding: 0 16px; height: 42px; color: var(--text-main);
            font-size: 14px; transition: all 0.2s;
        }
        .search-input:focus { border-color: var(--accent-blue); outline: none; box-shadow: 0 0 0 4px rgba(59, 130, 246, 0.1); }
        ...
        .metric-card {
            ...
            padding: 0; position: relative; overflow: hidden; height: 100%;
        ...
        .game-stat-item {
            background: var(--table-header-bg); border-radius: 12px;
            padding: 14px; margin-bottom: 12px; border: 1px solid var(--border-dim);
        }

        .section-title {
            display: flex; align-items: center; gap: 12px; font-size: 16px; font-weight: 800;
            color: var(--text-main); margin-bottom: 20px; text-transform: uppercase; letter-spacing: 0.5px;
        }
        .title-bar { width: 4px; height: 20px; border-radius: 2px; }
        ...
        <div style="padding: 24px 14px;">
            ...
            <div class="search-area">
                <form method="POST" class="search-input-group" action="index.php">
                    <input type="text" name="searchinp" placeholder="Search by ID, Game or User ID..." class="search-input" value="<?php echo htmlspecialchars($searched); ?>" />
                    ...
                    <div class="metric-card">
                        ...
                        <div class="metric-card-body">
                            <?php if (count($game_stats) > 0): ?>
                                <?php foreach ($game_stats as $game): ?>
                                    <div class="game-stat-item">
                                        <div class="game-header">
                                            <div class="game-name"><?php echo htmlspecialchars($game['game']); ?></div>
                                            ...
                                            <?php if (isset($top_players_by_game[$game['game']])): ?>
                                                <?php foreach ($top_players_by_game[$game['game']] as $player): ?>
                                                    <div class="player-item">
                                                        <div class="player-id"><?php echo htmlspecialchars($player['user_id']); ?></div>
                                ...
                                <?php endforeach; ?>
                            <?php else: ?>
                                <p class="text-center text-muted py-4">No data for today</p>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
                                    ...
                                    <?php if (count($todayTopBets) > 0): ?>
                                        <?php foreach ($todayTopBets as $bet): ?>
                                            <tr>
                                                ...
                                                <td style="font-weight: 700;">₹<?php echo number_format($bet['tbl_match_cost'], 2); ?></td>
                                            </tr>
                                        <?php endforeach; ?>
                                    <?php else: ?>
                                        <tr><td colspan="3" class="text-center py-4 text-muted">None today</td></tr>
                                    <?php endif; ?>
                                </tbody>
                            </table>
                        </div>
                    ...
                    <div class="metric-card">
                        ...
                        <div class="metric-card-body p-0">
                            <ul class="nav nav-tabs" id="yesterdayTabs" role="tablist">
                                <li class="nav-item">
                                    <button class="nav-link active" id="date-tab" data-bs-toggle="tab" data-bs-target="#date-content">By Date</button>
                                </li>
                                <li class="nav-item">
                                    <button class="nav-link" id="hours-tab" data-bs-toggle="tab" data-bs-target="#hours-content">By Hours</button>
                                </li>
                            </ul>
                                    ...
                                    <table class="r-table">
                                        ...
                                        <tbody>
                                            <?php if (count($yesterdayTopBets) > 0): ?>
                                                ...
                                                <?php endforeach; ?>
                                            <?php else: ?>
                                                <tr><td colspan="3" class="text-center py-4 text-muted">No high bets yesterday</td></tr>
                                            <?php endif; ?>
                                        </tbody>
                                    </table>
                                </div>
                                    ...
                                    <table class="r-table">
                                        ...
                                        <tbody>
                                            <?php if (count($alt_yesterday_bets) > 0): ?>
                                                ...
                                                <?php endforeach; ?>
                                            <?php else: ?>
                                                <tr><td colspan="3" class="text-center py-4 text-muted">No high bets in period</td></tr>
                                            <?php endif; ?>
                                        </tbody>
                                    </table>
                                </div>
            ...
            <div class="metric-card mb-4" style="background: var(--panel-bg);">
                <div class="section-title">
                    <span class="title-bar" style="background: linear-gradient(180deg, var(--accent-amber), #d97706);"></span>
                    ...
                    <table class="r-table">
                        ...
                        <tbody>
                            ...
                            while ($top = mysqli_fetch_assoc($top_bet_result)) {
                                ...
                                <tr>
                                    <td><?php echo $i; ?></td>
                                    <td style="font-weight: 700; color: var(--text-main);"><?php echo htmlspecialchars($top['tbl_user_id']); ?></td>
                                    <td><?php echo htmlspecialchars($top['tbl_project_name']); ?></td>
                                    <td style="font-weight: 700;">₹<?php echo number_format($top['tbl_match_cost'], 2); ?></td>
                                    <td style="font-weight: 700; color: <?php echo $is_profit ? 'var(--status-success)' : 'var(--status-danger)'; ?>">
            ...
            <div class="record-section">
                ...
                <div class="d-flex justify-content-between align-items-center mt-4" style="padding: 0 10px;">
                    <div style="font-size: 12px; color: var(--text-dim); font-weight: 600;">
                        Showing page <?php echo $page_num; ?> of <?php echo $total_p; ?> (Total: <?php echo $total_rec; ?> records)
                    </div>
                    <div class="pagination-container" style="margin: 0;">
                        ...
                        ?>
                        <a href="?page_num=<?php echo min($total_p, $page_num + 1); ?>" class="page-btn <?php if ($page_num >= $total_p) echo 'disabled'; ?>">
