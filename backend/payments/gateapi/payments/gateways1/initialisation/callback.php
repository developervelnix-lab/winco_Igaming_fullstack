...
function getConnection() {
    $conn = new mysqli("localhost", "winco", "winco", "winco");
...
if ($status === 'completed' || $status === 'success') {
    ...
    try {
        $stmt1 = $conn->prepare("UPDATE tblusersdata SET tbl_balance = tbl_balance + ?, tbl_requiredplay_balance = tbl_requiredplay_balance + ? WHERE tbl_uniq_id = ?");
        $stmt1->bind_param("dds", $money, $money, $userId);
        $stmt1->execute();
        $stmt1->close();
