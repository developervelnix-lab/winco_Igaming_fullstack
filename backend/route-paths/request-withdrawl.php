...
if (mysqli_num_rows($select_query) > 0) {
    ...
    $check_sql = "SELECT SUM(tbl_transaction_amount) AS total_transaction_amount FROM tblotherstransactions WHERE tbl_user_id = '{$const_user_id}' AND tbl_transaction_type = 'Play Matched'";
    $check_result = mysqli_query($conn, $check_sql);
    $check_row = mysqli_fetch_assoc($check_result);
    $total_transaction_amount = $check_row['total_transaction_amount'] ?? 0;

    if ($total_transaction_amount > 0) {
        $amount_sql = "SELECT SUM(tbl_match_cost) AS total_amount FROM tblmatchplayed WHERE tbl_user_id = '{$const_user_id}'";
        ...
        if ($total_transaction_amount > $total_amount) {
            $resArr['status_code'] = "gameplay_required";
            $resArr['required_play_balance'] = $total_transaction_amount - $total_amount;
            returnRequest($resArr);
        }
    }

    if ($required_play_balance > 0) {
        $resArr['status_code'] = "gameplay_required";
        $resArr['required_play_balance'] = $required_play_balance;
    } elseif ($total_num_withdraw >= $MAX_WITHDRAW_ALLOWED) {
        ...
        if ($decoded_password == 1) {
            if ($res_data['tbl_account_status'] == "true") {
                ...
                if ($insert_sql->error == "") {
                    $updated_balance = $available_balance - $const_withdraw_amount;
                    $updated_main_balance = $account_main_balance - $const_withdraw_amount;
                    ...
                    $update_sql->bind_param("ss", $updated_balance, $const_user_id);
                    $update_sql->execute();
