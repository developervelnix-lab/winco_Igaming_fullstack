...
?>
<tr<?php if ($row['tbl_match_status'] == "profit") echo ' class="approved_order"'; ?>>
  <td><?php echo $row['tbl_project_name']; ?></td>
  <td>₹<?php echo number_format($profit_amount, 2); ?></td>
  <td>₹<?php echo number_format($bet_amount, 2); ?></td>
  <td style="color: <?php echo $result_color; ?>;"><strong><?php echo $result_label; ?></strong></td>
  <td><?php echo $row['tbl_time_stamp']; ?></td>
</tr>
          ...
          <?php } ?>
            <tr>
  <td colspan="3">Total Bet Amount: ₹<?php echo number_format($total_amnt, 2); ?></td>
  <td colspan="2">Total Profit: ₹<?php echo number_format($total_profit_amnt, 2); ?></td>
</tr>
<?php }else{ ?>
            <tr>
...
<br><br>
<p class="cl-red">Sports Activities:</p>

<?php
// Extended sports filter to include LuckSportGaming
$sports_sql = "SELECT * FROM tblmatchplayed 
...
<div class="w-100 ovflw-x-scroll">
  <table id="sports_activities_table" class="cus-tbl mg-t-10 bg-white">
    <th colspan="5" style="background-color: #2F4F4F; color: white; text-align: center; padding: 12px; font-size: 18px;">
      Sports Activities (Saba Sports, Lucksport, LuckSportGaming)
    </th>
  </tr>
...
if (mysqli_num_rows($sports_result) > 0) {
  while ($row = mysqli_fetch_assoc($sports_result)) {
    ...
    if ($result_val > 0) {
      $sports_profit_amount += $result_val;
      $result_label = "Profit ₹" . number_format($result_val, 2);
      $result_color = "green";
    } elseif ($result_val < 0) {
      $sports_loss_amount += abs($result_val);
...
?>
  <tr style="font-weight: bold;">
    <td colspan="1">Total Sports Bet Amount: ₹<?php echo number_format($sports_total_bet, 2); ?></td>
    <td colspan="1">Total Sports P&L: ₹<?php echo number_format($sports_total_profit, 2); ?></td>
    <td colspan="1" style="color: red;">Sports Loss: ₹<?php echo number_format($sports_loss_amount, 2); ?></td>
    <td colspan="2" style="color: green;">Sports Profit: ₹<?php echo number_format($sports_profit_amount, 2); ?></td>
  </tr>
<?php } else { ?>
  <tr>
    <td colspan="5">No Sports Data Found!</td>
  </tr>
<?php } ?>
  </table>
</div>
...
<table id="match_summary_table" class="cus-tbl mg-t-10 bg-white">
 ...
 <th colspan="6" style="background-color: #8B4513; color: white; text-align: center; padding: 12px; font-size: 18px;">
...
while ($row = mysqli_fetch_assoc($result)) {
 ...
 if (!empty($grouped_data)) {
    ...
    $pnl_total = $data['profit_total'];
    $match_count = $data['count'];

    $result_amount = $pnl_total - $bet_total;
    $result_type = $result_amount > 0 ? 'Profit' : ($result_amount < 0 ? 'Loss' : 'Break Even');
    $result_color = $result_amount > 0 ? 'green' : ($result_amount < 0 ? 'red' : 'gray');

    echo "<tr>
            <td>{$index}</td>
            <td>{$date}</td>
            <td>₹" . number_format($bet_total, 2) . "</td>
            <td>₹" . number_format($pnl_total, 2) . "</td>
            <td style='color: {$result_color};'><strong>{$result_type}</strong><br>₹" . number_format(abs($result_amount), 2) . "</td>
            <td>{$match_count} match" . ($match_count > 1 ? 'es' : '') . "</td>
          </tr>";
    $index++;
}
