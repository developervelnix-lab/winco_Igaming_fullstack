...
class SmsManager {
    ...
    function checkOTPTimeGap(){
      ...
      $sql = "SELECT * FROM tblrecentotp WHERE tbl_mobile_num='{$this->const_inp_mobile}' ORDER BY id DESC LIMIT 1 ";
    ...
    private function checkOTPLimit(){
        ...
        $sql = "SELECT * FROM tblrecentotp WHERE tbl_mobile_num='{$this->const_inp_mobile}' AND tbl_otp_date='{$this->curr_date}' ";
        $sql_query = mysqli_query($this->conn, $sql);
        
        if (mysqli_num_rows($sql_query) < $this->const_day_otp_limit) {
            $return = true;
        }
